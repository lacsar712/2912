...
const RepairOrdersPage = {
    ...
    async loadEquipments() {
        try {
            const response = await ProductionService.getEquipments({ size: 200 });
            if (response.code === 200) {
                this.equipments = response.data.items || [];
                RepairCreateModal.setEquipmentsCache(this.equipments);
            }
        } catch (e) {
    ...
    async loadOrders() {
        try {
            const response = await RepairService.getOrders({ size: 200 });
            if (response.code === 200) {
